...
const RecruiterDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5007/api/recruiters/filter", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ year, filter: { company } }),
    })
      .then(res => res.json())
      .then(data => {
  ...
  const paginated = placements.slice((currentPage - 1) * pageSize, currentPage * pageSize);

  const handleDownloadCSV = () => {
    const headers = ["Name", "Reg. No", "Offer"];
    const rows = placements.map(p => [p.name, p.registration, p.offer]);
    ...
    link.href = url;
    link.setAttribute("download", `Recruiter_${company}_${year}.csv`);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {placements.length === 0 ? (
          <p style={styles.empty}>No placements found for {company} in {year}.</p>
        ) : (
          <>
                ...
                <tbody>
                  {paginated.map((p, i) => (
                    ...
                    >
                      ...
                      <td style={styles.td}>{p.registration}</td>
                      <td style={styles.td}>{p.offer}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

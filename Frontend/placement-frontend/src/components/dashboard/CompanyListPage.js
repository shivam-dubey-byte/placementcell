...
const CompanyListPage = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5007/api/companies/filter", {
      method: "POST",
      headers: {
  ...
  const handleCompanyClick = (companyName) => {
    navigate(`/dashboard/recruiter/${year}?company=${encodeURIComponent(companyName)}`);
  };

  const handleDownloadCSV = () => {
    const headers = ["Company", "Students"];
    const rows = companies.map(c => [c.company, c.students]);
    ...
    link.href = url;
    link.setAttribute("download", `Companies_${year}.csv`);
      ...
      <div style={styles.card}>
        <div style={styles.headerRow}>
          <h2 style={styles.headingText}>
            Companies Visited — <span style={styles.yearBadge}>{year}</span>
          </h2>
        ...
        {companies.length === 0 ? (
          <p style={styles.empty}>No company data found for {year}.</p>
        ) : (
          <>
              ...
              <table style={styles.table}>
                <thead>
                  <tr>
                    ...
                    <th style={styles.th}># of Students</th>
                  </tr>
                </thead>
                <tbody>
                  {paginated.map((c, i) => (

...
const OfferDetailsPage = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5007/api/placements/filter", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ year, offer: type }),
    })
      .then(res => res.json())
      .then(data => {
  ...
  const handleDownloadCSV = () => {
    const headers = ["Name", "Reg. No", "Company"];
    const rows = placements.map(p => [p.name, p.registration, p.company]);
    ...
    link.href = url;
    link.setAttribute("download", `Offer_${type}_${year}.csv`);
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        {placements.length === 0 ? (
          <p style={styles.empty}>No placement data found for this offer type.</p>
        ) : (
          <>
